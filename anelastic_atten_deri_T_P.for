...
C     LitMod_v4.0 (MF)
C
      SUBROUTINE anelastic_atten_deri_T_P
c**********************************************************************
...
c reading into arrays
     ...
     *  d(i,6),d(i,7),d(i,8),d(i,9),d(i,10)
      enddo
1010  format(F9.3,1F17.4,1F15.3,2F12.3,4E17.6,1F8.2)
c @ ajay
...
c    dT
      do i=1,400
        do j=1,300
            dVp_dT(j,i)=(Vp_mat(j,i)-Vp_mat(j+1,i))/(T(j,i)-
     *  T(j+1,i))
            if (dVp_dT(j,i)==0.0) then
                dVp_dT(j,i)=dVp_dT(j-1,i)
            endif
            dVs_dT(j,i)=(Vs_mat(j,i)-Vs_mat(j+1,i))/(T(j,i)-
     *  T(j+1,i))
            if (dVs_dT(j,i)==0.0) then
                dVs_dT(j,i) = dVs_dT(j-1,i)
           endif
c        write(*,*)dVs_dT(j,i),P(j,i),T(j,i)
        enddo
       enddo    
...
c     *  dVp_dT(i,j),dVs_dT(i,j),dVp_dP(i,j),dVs_dP(i,j)
     *  d(index,5),
     *  dVp_dT(i,j)+d(index,6),dVs_dT(i,j)+d(index,7),
     *  dVp_dP(i,j)+d(index,8),dVs_dP(i,j)+d(index,9),d(index,10)
c     *  d(index,6),d(index,7),d(index,8),d(index,9),d(index,10)   
